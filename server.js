...
app.post('/api/recharge', async (req, res) => {
  ...
  const { data: agent, error: agentErr } = await supabaseAdmin.from('users').select('id, role').eq('username', agentUsername).eq('role', 'recharge_agent').single()
  if (agentErr || !agent) return res.status(400).json({ error: 'وكيل الشحن غير موجود' })
  const { data: user } = await supabaseAdmin.from('users').select('coins, total_recharged').eq('id', userId).single()
  const newCoins = (user?.coins || 0) + amount
  const newTotal = (user?.total_recharged || 0) + amount
  await supabaseAdmin.from('users').update({ coins: newCoins, total_recharged: newTotal }).eq('id', userId)
...
app.post('/api/send-gift', async (req, res) => {
  ...
  const { data: user, error: userErr } = await supabaseAdmin.from('users').select('*').eq('id', userId).single()
  if (userErr || !user) return res.status(400).json({ error: 'مستخدم غير موجود' })
  const { data: host, error: hostErr } = await supabaseAdmin.from('users').select('*').eq('username', hostUsername).single()
  if (hostErr || !host || host.role !== 'host') return res.status(400).json({ error: 'المضيف غير موجود' })
  ...
  const newDiamonds = host.diamonds + price
  await supabaseAdmin.from('users').update({ diamonds: newDiamonds }).eq('id', host.id)
  await supabaseAdmin.from('transactions').insert({ from_user: userId, to_user: host.id, amount: price, type: 'gift' })
  let luckyWin = 0
  if (gift.is_lucky) {
    const { data: settings } = await supabaseAdmin.from('settings').select('value').eq('key', 'daily_luck_percent').single()
    const luckPercent = parseInt(settings?.value || 10)
    const random = Math.random() * 100
    if (random <= luckPercent) {
  ...
  if (host.host_agency_id) {
    ...
    if (agency) {
      ...
      if (commission > 0) {
        const { data: agentUser } = await supabaseAdmin.from('users').select('diamonds').eq('id', agency.agent_id).single()
        if (agentUser) {
          await supabaseAdmin.from('users').update({ diamonds: agentUser.diamonds + commission }).eq('id', agency.agent_id)
        }
...
app.post('/api/admin', async (req, res) => {
  const { action, table, match, data: payload, order, limit, offset, single, count } = req.body
  ...
  if (action === 'insert') {
    const result = await supabaseAdmin.from(table).insert(payload).select()
  ...
  if (action === 'update') {
    ...
    if (match) {
      for (const [k, v] of Object.entries(match)) {
        if (typeof v === 'object' && v.op) q = q[v.op](k, ...(v.args || [v.val]))
        else q = q.eq(k, v)
      }
    }
  ...
  if (action === 'upsert') {
    const result = await supabaseAdmin.from(table).upsert(payload)
    return res.json({ data: result.data, error: result.error })
  }
  if (action === 'delete') {
...
app.post('/api/admin/reports', async (req, res) => {
  ...
  const totalRecharged = recharges.reduce((a, b) => a + (b.amount || 0), 0)
